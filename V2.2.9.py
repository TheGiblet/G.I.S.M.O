...
# - I2S audio output using a Daokai MAX98357 I2S Class D amplifier and speaker (currently disabled)
# - Touch sensor on GPIO 22 to trigger a "shake" behavior (with debouncing)
# - Camera for capturing images and video (using libcamera)
...
picam2 = Picamera2()
video_config = picam2.create_video_configuration(main={"size": (640, 480)})  # Set the size directly
...
# --- Function to play a sound file (currently disabled) ---
...
try:
    while True:
        ...
        if left_ir_state == 1 and right_ir_state == 1:  # Both sensors off the line (edge detected)
            print("Edge detected!")
            play_sound("edge.wav")  # Play a sound for edge detection (disabled for now)
        ...
        # --- Obstacle avoidance (only if not at an edge) ---
            ...
            play_sound("obstacle.wav")  # Play a sound for obstacle detection (disabled for now)
        ...
        if GPIO.input(in1) == GPIO.LOW and GPIO.input(in2) == GPIO.HIGH and \
           GPIO.input(in3) == GPIO.LOW and GPIO.input(in4) == GPIO.HIGH:  # Check if motors are in reverse
            ...
            if rear_ir_state == 0:  # Obstacle detected at the rear
                ...
                play_sound("rear_obstacle.wav")  # Play a sound for rear obstacle detection (disabled for now)
                stop_motors()
                time.sleep(0.5)
        ...
        # --- Camera functionality (using picamera2) ---
        video_path = '/home/pi/Desktop/video.h264'
        encoder = picam2.encoders[picam2.video_configuration.main.format]  # Get the encoder directly
